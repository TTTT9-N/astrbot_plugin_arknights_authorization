...
- `resource_index_service.py`：资源盲盒索引生成（用于市场/回收逐盒定价）
...
- 回收系统：`/方舟盲盒 回收 <种类ID>` 先查看该类可回收物品与当前单价，再使用 `/方舟盲盒 回收 <种类ID> <奖品名> [数量]` 回收并返还余额。
...
- 市场与回收价格改为“同种类内每个盲盒独立定价”，例如一个种类 14 盒会分别计算 14 个价格。
